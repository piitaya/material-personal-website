(function () {
	...
	function AdminExperienceTypesComponent($mdDialog, $mdToast, ExperienceType) {
		...
		vm.new = function(event) {
		  ...
		  .then(function(newType) {
				ExperienceType.create(newType).$promise.then(function(result) {
					...
					$mdToast.show(
						$mdToast.simple()
							.textContent("Type d'experience créée.")
							.position("bottom right")
						);
				}).catch(function(err) {
					$mdToast.show(
						$mdToast.simple()
							.textContent("Erreur lors de la création du type d'experience.")
							.position("bottom right")
					);
				});
		  }, function() {
        ...
        .title("Êtes-vous sûr de supprimer ce type d'experience?")
        .textContent('Cette action est définitive.')
        .ariaLabel("Suppression de type d'experience")
        .targetEvent(event)
        .ok('Oui')
        .cancel('Non');
		  $mdDialog.show(confirm).then(function() {
	    	...
	    	ExperienceType.deleteById({id: type.id}).$promise.then(function() {
				...
				$mdToast.show(
					$mdToast.simple()
						.textContent("Type d'experience supprimée.")
						.position("bottom right")
					);
				}).catch(function(err) {
					$mdToast.show(
						$mdToast.simple()
							.textContent("Erreur lors de la suppression du type d'experience.")
							.position("bottom right")
					);
				});
		  }, function() {
